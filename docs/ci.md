...
|:------------------------------------------------------------------------|:----------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------|
| [pull-request.yml](../.github/workflows/pull-request.yml)               | `pull_request`, whatever branch it targets                            | Runs the checks, publishes a preview image and runs the end-to-end tests against it.                                            |
| [pull-request-opened.yml](../.github/workflows/pull-request-opened.yml) | `pull_request` limited to `opened`, `reopened` and `ready_for_review` | Turns auto-merge on, unless the pull request is part of a stack.                                                                |
...
There is no merge queue: a pull request merges once its `CI` check has passed on the head commit and a code owner has approved that commit, see [Reviews and merging](#reviews-and-merging). None of the three pull request workflows filters on the branch a pull request targets. A pull request in a [stack](#stacked-pull-requests) targets the branch of the pull request below it. GitHub runs the workflows for it as if it targeted `main`, but only once it is linked into the stack, which happens after it opens. Without the filter, every pull request gets its checks and its preview image from the moment it opens.
...
[pull-request.yml](../.github/workflows/pull-request.yml) runs four jobs on every pull request, whatever branch it targets.
...
`Detect changes` runs alongside `CI`. It checks the repository out and runs [detect-changes](#detect-changes), and its `preview` output decides whether the pull request gets a preview image. The output is the filter's `image` result, or `true` for a pull request in a [stack](#stacked-pull-requests), which the payload marks with a `stack` object. Such a pull request's app is the layers below plus its own, and `dev` serves neither until those layers have merged and [push-main.yml](#pushes-to-main) has rebuilt it.

### Build image

`Build image` needs `Detect changes` and runs when that job reports a preview image and the head branch belongs to this repository; a fork cannot read the registry credentials, and its Dockerfile is not ours to publish. It hands [build-image](#build-image) the username variable, `DOCKERHUB_TOKEN` and the tag rule `type=ref,event=pr`, so the image lands as `dndmapp/web-app:pr-<N>`, with `N` the pull request number. A pull request outside a stack that leaves the image inputs alone skips the build.

### End-to-end tests

`End-to-end tests` needs `Detect changes` and `Build image`. It checks the repository out, runs [setup-workspace](#setup-workspace) and hands [run-e2e](#run-e2e) the tag to serve, the username variable and `DOCKERHUB_READ_TOKEN`. The tag is `pr-<N>` when `Build image` succeeded and `dev` when `Detect changes` reported no preview image. It only reports none for a pull request outside a stack that leaves the image inputs alone, so the pull request's app is then the one `main` already serves. Any other outcome skips the job: a failed build, a failed filter, or a fork. `needs` alone would skip the job whenever `Build image` is skipped, so the condition starts with `!cancelled()`, which gets it evaluated regardless. It also requires `Detect changes` to have succeeded, since the `preview` output of a failed filter would read `false` rather than nothing. The `Default branch` ruleset requires the check beside `CI` and `Build image`, and a skipped job passes a required check, so a pull request that skips it still merges.

## Auto-merge

[pull-request-opened.yml](../.github/workflows/pull-request-opened.yml) turns auto-merge on for every pull request outside a stack, so GitHub merges it itself once the `Default branch` ruleset is satisfied: one approving review from a code owner on the last push and a passing `CI` check. Its single `Enable auto-merge` job runs `gh pr merge --auto --merge` against the pull request URL, so it needs no checkout, and `--merge` matches the one merge method the ruleset allows. The job runs on `reopened` as well, because closing a pull request turns auto-merge off, and on `ready_for_review`, because it skips drafts: GitHub does not enable auto-merge on one. It also skips pull requests from forks, which cannot read the app credentials. `gh` merges on the spot when nothing is left to wait for, so a reopened pull request that still has an approval and a green check does not stall. Pushing to the head branch dismisses the existing approval, so auto-merge waits for a fresh one; enabling it is a one-time step per pull request that the pushes do not undo.

The job also skips a pull request that belongs to a [stack](#stacked-pull-requests), since GitHub does not support auto-merge on one and the mutation fails with `Auto-merge is not supported for stacked pull requests`. The `pull_request` payload carries a `stack` object for exactly those pull requests, so the job's condition tests `github.event.pull_request.stack == null` and swallows no other error. The test only holds once the stack exists on GitHub, and `gh stack submit` creates the pull requests before it links them into a stack. A stack's pull requests therefore open as drafts, which is what `gh stack submit --auto` does, so the job first runs on `ready_for_review`, when the stack is in place; see [Stacked pull requests](commits.md#stacked-pull-requests). A pull request the job skipped is merged by its author, see [Reviews and merging](#reviews-and-merging).
...
[pull-request-closed.yml](../.github/workflows/pull-request-closed.yml) removes the preview image once the pull request closes, merged or not, through its single `Delete image` job. The job runs for every closed pull request from a branch of this repository, the ones `Build image` builds for, and does not decide again whether the pull request got an image. It cannot: a pull request in a [stack](#stacked-pull-requests) gets its image whatever its files, a stack can be taken apart again, and the filter's own answer moves with the base branch GitHub retargets. The rule that published the tag may therefore no longer hold when the pull request closes. Asking Docker Hub is cheaper than reconstructing that history, and the job needs no checkout.

`Delete image` calls the Docker Hub API with `curl`. It trades the username variable and `DOCKERHUB_ADMIN_TOKEN` for a short-lived token at `/v2/auth/token`, then sends `DELETE /v2/repositories/dndmapp/web-app/tags/pr-<N>/`, the endpoint [docker/hub-tool](https://github.com/docker/hub-tool) uses, since Docker Hub documents no tag deletion. A 404 counts as done, because it means no build ever pushed the tag or an earlier run already removed it. The job shares the `pull-request-build-image-<N>` concurrency group with `Build image`, so closing a pull request mid-build cancels the build before the tag is removed, and a build that starts on reopening waits for the removal to finish.
...
Whenever auto-merge is not on for a pull request, its author is responsible for merging it once the ruleset is satisfied. The reason does not matter: the pull request is part of a [stack](#stacked-pull-requests), the `Enable auto-merge` job failed, or someone turned auto-merge off by hand. The one exception is an author without write access, such as the author of a pull request from a fork, whose pull request a maintainer merges; see [Pull requests](../CONTRIBUTING.md#pull-requests). A merge by a user starts [push-main.yml](#pushes-to-main) just as a merge by the app does, so nothing is lost by pressing the button.
...
[Stacked pull requests](commits.md#stacked-pull-requests) describes what a stack is and how one is created. GitHub enforces the `Default branch` ruleset on every pull request in the stack, so each one needs its own approval and its own green checks, and it runs the `pull_request` workflows for each of them as if it targeted `main`. The workflows above treat a stack as follows:

- **Checks.** Every pull request in the stack gets the [Pull requests](#pull-requests) jobs on every push, from the moment it opens, since none of the workflows filters on the branch a pull request targets. The `CI` job lints the commits between the pull request's base and its head, so each pull request is checked for its own layer only.
- **Preview image.** Every pull request in the stack gets a preview image, whether or not its own layer touches the image inputs, and the end-to-end tests run against it. `dev` serves the layers below only once they have merged and [push-main.yml](#pushes-to-main) has rebuilt it, which is later than the next run of the pull request above them.
- **Auto-merge.** None. GitHub does not support auto-merge on a stacked pull request, so the [Enable auto-merge](#auto-merge) job skips it and the author merges.

A stack merges from any of its pull requests, from the bottom up. Merging one merges it together with every unmerged pull request below it, in one operation, and only once each of those is approved and has passing checks. So the whole stack merges in one go from its top, or in parts, one or more pull requests at a time, but never a pull request without the ones below it. `gh stack merge <pr-number>` merges up to and including a pull request, and `gh stack merge` alone asks how far up to merge. The merge button on a pull request does the same as the command for that pull request. The ruleset allows merge commits only, so each pull request lands as its own merge commit, bottom first, as if merged one at a time.

After a partial merge, GitHub retargets the lowest remaining pull request onto `main` and rebases its branch, which force pushes it and attributes that push to whoever performed the merge. The ruleset requires the approval to come from someone other than the last pusher, so the author merges and the `dnd-mapp-bot` account, or another code owner, approves the rebased head. The other way around does not work: an approval by the account that merged the layer below is an approval by the last pusher, and it does not count. GitHub has also acknowledged a bug in how it evaluates approvals when merging a stack under that setting; see [github/gh-stack#212](https://github.com/github/gh-stack/discussions/212). A stack that stays blocked with every approval in place has hit it. Until GitHub fixes it, a maintainer turns `Require approval of the most recent reviewable push` off in the `Default branch` ruleset for the merge and back on right after.
