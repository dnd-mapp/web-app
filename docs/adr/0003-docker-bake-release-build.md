...
Base image references stay as `tag@sha256:digest` on the Dockerfile `FROM` lines. The bake file does not pass `NODE_IMAGE` / `NGINX_IMAGE` (or `PNPM_VERSION`) as build args, even though the issue that requested this file (#3) asked for a "base-image digest passthrough so the values stay defined in one place". Routing an image through a bake variable and `FROM ${IMAGE}` is exactly the form Dependabot cannot resolve (`0002-track-base-images-with-dependabot.md`).
...
The release tag scheme — `edge` for the tip of `main` plus an immutable `sha-<short>` tag, with `latest` and the semver tag deferred until the first `v*` git tag — is owned by #5, where `docker/metadata-action` produces the `TAGS` value. `edge` is chosen over `latest` (which Docker's own guidance says carries no "newest" guarantee and which projects like nginx wire to a *less* stable line), over the bare branch name `main`, and over `next` (a version-stream label, not a branch pointer): Alpine and `docker/metadata-action` both define `edge` as "the last commit of the active branch", so it states exactly what this build is.

## Considered options

- **Pass base-image digests through bake.** Rejected: contradicts `0002` — Dependabot stops seeing the images.
...
- **`local` inheriting `default` and disabling attestations.** Rejected: `attest = []` and the `provenance = false` / `sbom = false` shorthands do not override an inherited `attest` list; only re-listing every entry with `disabled=true` works. A shared `_common` parent is simpler.
