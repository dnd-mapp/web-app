...
The `web-app` application lives under `projects/web-app/src`, divided into areas. `core` holds what bootstraps the application: the root component and its configuration. `shared/components` holds the components any page may use, such as the bar across the top, and the ones that dress up a native element through an attribute selector, such as the button. `shared/localization` holds the texts the interface is written in and the providers that resolve them, described in [Localization](localization.md). `shared/testing` holds the helpers specs share, described under [Unit tests](testing.md#unit-tests); it is a `testing/` folder itself, so it stays out of the application build. Each area exposes its public symbols through an `index.ts` barrel, and `paths` in `tsconfig.json` maps `@/<area>` onto that barrel, so code outside the area imports from `@/core`, `@/components` or `@/testing` rather than through a relative path. The alias takes the last folder of the area, so `shared/` groups areas on disk without appearing in imports. A new area is complete once it has a barrel and an alias for it, plus a `@/<area>/testing` alias when it ships harnesses. File and class naming follows the `schematics` defaults in `angular.json`: `root.component.ts` holds `RootComponent`, styles are SCSS, and guards, interceptors, pipes and resolvers use a `.` type separator. Beside the areas, one folder holds no code and so has no barrel or alias: `design-system`, which holds the SCSS described in [Design system](design-system.md).
