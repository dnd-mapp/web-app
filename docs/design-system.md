...
A color scale enters the primitive layer whole, all eleven steps, so a semantic token picks from a fixed palette rather than introducing a shade of its own. Every other token is added when a style needs the value, not ahead of it: the typography primitives and the semantic tokens describe what the application uses today, so an unused one is removed rather than kept for later. To add one, put the raw value in the primitive layer under a name that describes the value, add a semantic token that names the purpose and points at the primitive, and reference the semantic token from the style. When a value already exists as a primitive, only the semantic token is new. [Stylelint](linting.md#styles) checks the partials along with the rest of the SCSS.
