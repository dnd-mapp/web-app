...
[release.yml](../.github/workflows/release.yml) runs on a `push` of a tag matching `v<major>.<minor>.<patch>`, through four jobs. The trigger glob accepts nothing with a pre-release or build suffix, but it cannot reject a leading zero, so the first job starts by checking the tag against the strict pattern.
...
`Deploy to production` needs `Build image` alone. It checks the repository out and hands the [deploy](ci.md#deploy) action the tag `latest`, so the production environment serves the release as soon as its image is on Docker Hub; see [Environments](ci.md#environments). It runs beside the release below rather than ahead of it: the image is published either way, so a deployment that fails should not hold the announcement back. Rerunning this job alone puts the release live.

`Create the GitHub release` needs `Verify the release` and `Build image` and runs [softprops/action-gh-release](https://github.com/softprops/action-gh-release) with `Release <tag>` as the name and the notes as the body, once the image the release announces is on Docker Hub. It is the one job with `contents: write`, and it runs nothing but that action, without a checkout; the build runs the Dockerfile and every dependency under `contents: read`. The workflow token is enough, since no workflow listens for the `release` event.

The checks are not run again: the tagged commit is a `main` commit that [push-main.yml](ci.md#pushes-to-main) already checked. All four jobs use fixed concurrency groups without canceling, so releases run one at a time in push order, `latest` ends up on the newest tag and production serves it.
