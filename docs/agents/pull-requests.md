...
A pull request that references an issue moves that issue's card to `In review` on the [D&D Mapp project](https://github.com/orgs/dnd-mapp/projects/8). Merging a pull request that closes an issue moves it to `Done`. Pull requests are not added to the board themselves.
