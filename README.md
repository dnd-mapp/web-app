...
GitHub Actions runs the checks on every pull request and again in the merge queue, through [pull-request.yml](.github/workflows/pull-request.yml) and [merge-group.yml](.github/workflows/merge-group.yml); a third workflow, [pull-request-closed.yml](.github/workflows/pull-request-closed.yml), cleans up after a pull request closes and is described below. Both hand their setup to the [setup-workspace](.github/actions/setup-workspace/action.yml) composite action, which installs Node.js, pnpm and the dependencies in one step with [pnpm/setup](https://github.com/pnpm/setup). That action reads `devEngines` from [package.json](package.json), so CI uses the versions listed under [Prerequisites](#prerequisites), and it installs from the lockfile with `--frozen-lockfile`.
...
Commit messages are the exception. A `Lint commit messages` step sits in [pull-request.yml](.github/workflows/pull-request.yml) itself, between the setup and the checks, and runs commitlint over `base..head`, which holds the commits the pull request adds and nothing else. It stays out of [run-checks](.github/actions/run-checks/action.yml) because it needs something the other checks do not: a commit range, and the two ends of that range live in the `pull_request` payload, which a merge queue run does not have. The checkout in that workflow therefore sets `fetch-depth: 0`, since the default shallow fetch leaves out the commits commitlint has to read. Checking on the pull request alone loses nothing, because the merge queue entry holds the same commits that were already checked, and since the step sits inside the existing job, both workflows report the same `CI` check.

[pull-request.yml](.github/workflows/pull-request.yml) carries two more jobs, which publish a preview image of the pull request. `Detect image changes` runs the [detect-changes](.github/actions/detect-changes/action.yml) composite action, which hands the files the pull request changes to [dorny/paths-filter](https://github.com/dorny/paths-filter), against patterns that mirror [.dockerignore](.dockerignore) plus the files that decide whether and how the image is built: the [Dockerfile](Dockerfile), [docker-bake.hcl](docker-bake.hcl), the workflow and the action itself. The action reports one output per filter, `image` being the only one so far, so other workflows can share it for their own change detection. The filter reads the changed files through the GitHub API; the job checks the repository out only so the local action is on disk, and it runs alongside `CI`. `Build image` waits for it and runs when it reports a match and the pull request comes from a branch of this repository; a fork cannot read the registry credentials. The job logs in to Docker Hub with the `DOCKERHUB_USERNAME` variable, which holds `dndmapp` and is no secret since it is also the image's namespace, and the `DOCKERHUB_TOKEN` organization secret, which holds the Docker Hub access token named `github dnd-mapp organization workflows` on the `dndmapp` account. That token needs read, write and delete access: write to push the preview image, delete to remove it again. The secret is shared with the repositories that push images, and replacing the token on expiry is one edit in the organization settings. It then builds the image for `linux/amd64` and `linux/arm64` and pushes it as `dndmapp/web-app:pr-<N>`, with `N` the pull request number, through the steps described under [Docker image](#docker-image). A pull request that leaves the image inputs alone skips the build.

[pull-request-closed.yml](.github/workflows/pull-request-closed.yml) removes that image once the pull request closes, whether it was merged or not. Its `Detect image changes` job runs the same composite action, so the two workflows can never disagree about which pull requests get an image, and checks out the pull request's head commit rather than the merge commit, which GitHub no longer provides for a closed pull request that conflicts with its base. `Delete image` waits for it, runs under the same two conditions as `Build image`, and calls the Docker Hub API with `curl`: it trades the variable and secret for a short-lived token at `/v2/auth/token`, then sends `DELETE /v2/repositories/dndmapp/web-app/tags/pr-<N>/`, the endpoint [docker/hub-tool](https://github.com/docker/hub-tool) uses, since Docker Hub documents no tag deletion. A 404 counts as done, because it means the build never pushed the tag or an earlier run already removed it. The job shares its concurrency group with `Build image`, so closing a pull request while its image is still building cancels the build first, and reopening it queues the new build behind the removal.
