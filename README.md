...
[ESLint](https://eslint.org) checks every TypeScript file and component template under `projects/`, plus every JavaScript file in the workspace, against [eslint.config.js](eslint.config.js), through the `lint` target that [angular-eslint](https://github.com/angular-eslint/angular-eslint)'s builder provides in [angular.json](angular.json). TypeScript files, including the `.mts` and `.cts` variants, get ESLint's recommended rules, the type-checked `recommended` and `stylistic` sets from [typescript-eslint](https://typescript-eslint.io), and angular-eslint's recommended rules. The latter also pin component selectors to `app-` element selectors in kebab-case and directive selectors to `app` attribute selectors in camelCase, matching `prefix` in [angular.json](angular.json). Templates, whether in an `.html` file or inline in a component, get angular-eslint's recommended template rules plus its accessibility rules. The type-checked rules read types through the TypeScript project service, so a file has to be included by one of the tsconfig files: [tsconfig.app.json](projects/web-app/tsconfig.app.json) covers the application code and [tsconfig.spec.json](projects/web-app/tsconfig.spec.json) covers the specs and the `testing/` folders. Their `include` patterns only match `.ts`, so an `.mts` or `.cts` file goes under `files`, the way [vitest.config.ts](projects/web-app/vitest.config.ts) does; a file no tsconfig includes fails the lint with a parsing error instead of being skipped. JavaScript files (`.js`, `.mjs` and `.cjs`) get ESLint's recommended rules without type information, plus the Node.js globals from [globals](https://github.com/sindresorhus/globals). They are tooling config such as `eslint.config.js` itself, which runs in Node.js and which no tsconfig includes. They also sit at the workspace root rather than under a project, so the lint target's JavaScript pattern spans the whole workspace while the TypeScript and template patterns stay scoped to `projects/web-app`. The `angular-eslint` collection is listed first under `schematicCollections` in angular.json, so `ng generate application` and `ng generate library` add a `lint` target and a project-level ESLint config to every new project. The other generators fall through to the Angular defaults. Run `pnpm run lint-ts --fix` to apply every fix the rules can make on their own. The [vscode-eslint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) extension reads the same config and flags violations while you type.
...
[lefthook](https://lefthook.dev) installs the `pre-commit` hook defined in [lefthook.yml](lefthook.yml) as part of `pnpm install`, through its `postinstall` script; that is why `lefthook` is the one package allowed to run build scripts in [pnpm-workspace.yaml](pnpm-workspace.yaml). The hook runs Prettier over the staged files and stages what it rewrote, then runs ESLint, Stylelint and markdownlint-cli2 in parallel, each over the staged files of the types it covers. A finding from any of them stops the commit. It skips itself during merges and rebases, and the `postinstall` script does nothing when `CI` is set, so neither GitHub Actions nor the Docker build installs hooks. The command above runs the hook over whatever is staged without committing. The hook is a shortcut, not the gate: `git commit --no-verify` bypasses it, and [Continuous integration](#continuous-integration) runs the same tools over the whole repository.
